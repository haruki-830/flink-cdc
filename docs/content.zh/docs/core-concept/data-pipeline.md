...
| `schema.compatibility.mode`  | 如何协调 Pipeline Schema 与已存在的目标表。`SINK_DEFINED`（默认值）保持 Sink 原有行为；`RECONCILE` 在处理 `CreateTableEvent` 时尝试将缺失的非键列按 nullable 补齐，并安全扩宽过窄的目标列类型。不支持、不安全或执行失败的修复不会触发框架 fail-fast，而是交给 Sink 原有逻辑处理。 | optional          |
