...
| `schema.compatibility.mode`  | How to coordinate the pipeline schema with an existing target table. `SINK_DEFINED` (default) preserves the sink's behavior. During `CreateTableEvent` handling, `RECONCILE` tries to add missing non-key columns as nullable columns and safely widen narrow target column types. Unsupported, unsafe, or failed repairs are delegated to the sink without a framework fail-fast. | optional          |
